"""完整的朋友圈发布流程测试 v2"""
import sys
import io
import time
import os
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

import uiautomation as auto
from uiautomation import Keys

print("=" * 50)
print("完整朋友圈发布流程 v2")
print("=" * 50)

# 图片文件夹路径
image_folder = r"D:\苹果哥的商业系统A\电商\产品素材\待发布\十一月\2025-11-25"

def wait_and_find(control_func, timeout=5, interval=0.5):
    """等待并查找控件"""
    start = time.time()
    while time.time() - start < timeout:
        ctrl = control_func()
        if ctrl and ctrl.Exists(0.1, 0):
            return ctrl
        time.sleep(interval)
    return None

# Step 1: 找到微信主窗口
print("\n--- Step 1: 找到微信主窗口 ---")
main_window = auto.WindowControl(searchDepth=1, ClassName="mmui::MainWindow")
if not main_window.Exists(5, 1):
    print("[X] 未找到微信窗口")
    exit(1)
print(f"[OK] 找到微信窗口: {main_window.Name}")
main_window.SetFocus()
time.sleep(0.3)

# 关闭可能存在的朋友圈窗口和对话框
print("\n--- 清理现有窗口 ---")
for _ in range(3):
    sns = auto.WindowControl(searchDepth=1, ClassName="mmui::SNSWindow")
    if sns.Exists(0.5, 0):
        sns.SendKeys("{Escape}")
        time.sleep(0.3)

    dialog = auto.WindowControl(searchDepth=1, ClassName="#32770")
    if dialog.Exists(0.3, 0):
        dialog.SendKeys("{Escape}")
        time.sleep(0.3)

time.sleep(0.5)

# Step 2: 打开朋友圈窗口
print("\n--- Step 2: 打开朋友圈窗口 ---")
main_window.SetFocus()
time.sleep(0.3)

moment_btn = main_window.ButtonControl(searchDepth=10, Name="朋友圈")
if not moment_btn.Exists(3, 1):
    print("[X] 未找到朋友圈按钮")
    exit(1)

# 双击打开独立窗口
rect = moment_btn.BoundingRectangle
center_x = (rect.left + rect.right) // 2
center_y = (rect.top + rect.bottom) // 2
print(f"朋友圈按钮位置: ({center_x}, {center_y})")

auto.Click(center_x, center_y)  # 朋友圈按钮中心坐标
time.sleep(0.2)
auto.Click(center_x, center_y)  # 朋友圈按钮中心坐标
print("[OK] 已双击朋友圈按钮")
time.sleep(2)

# 等待朋友圈窗口
sns_window = wait_and_find(lambda: auto.WindowControl(searchDepth=1, ClassName="mmui::SNSWindow"), timeout=5)
if not sns_window:
    print("[X] 朋友圈窗口未打开")
    exit(1)
print(f"[OK] 朋友圈窗口已打开")
sns_window.SetFocus()
time.sleep(0.5)

# Step 3: 点击发表按钮
print("\n--- Step 3: 点击发表按钮 ---")
publish_btn = sns_window.Control(searchDepth=10, Name="发表")
if not publish_btn.Exists(3, 1):
    print("[X] 未找到发表按钮")
    exit(1)

rect = publish_btn.BoundingRectangle
center_x = (rect.left + rect.right) // 2
center_y = (rect.top + rect.bottom) // 2
auto.Click(center_x, center_y)  # 发表按钮中心坐标
print(f"[OK] 已点击发表按钮 ({center_x}, {center_y})")
time.sleep(1.5)

# Step 4: 点击添加图片按钮
print("\n--- Step 4: 点击添加图片按钮 ---")

# 等待添加图片按钮出现
add_image_btn = wait_and_find(
    lambda: sns_window.ListItemControl(searchDepth=15, Name="添加图片"),
    timeout=5
)

if not add_image_btn:
    print("[X] 未找到添加图片按钮")
    # 打印当前窗口内容
    print("当前窗口元素:")
    for i in range(1, 10):
        item = sns_window.ListItemControl(searchDepth=15, foundIndex=i)
        if item.Exists(0.2, 0):
            print(f"  ListItem [{i}]: {item.Name} | {item.ClassName}")
        else:
            break
    exit(1)

rect = add_image_btn.BoundingRectangle
center_x = (rect.left + rect.right) // 2
center_y = (rect.top + rect.bottom) // 2
print(f"添加图片按钮位置: ({center_x}, {center_y})")

# 点击按钮
sns_window.SetFocus()
time.sleep(0.2)
auto.Click(center_x, center_y)  # 添加图片按钮中心坐标
print("[OK] 已点击添加图片按钮")
time.sleep(2)

# Step 5: 查找并操作文件对话框
print("\n--- Step 5: 选择图片 ---")

# 等待文件对话框
file_dialog = wait_and_find(
    lambda: auto.WindowControl(searchDepth=1, ClassName="#32770"),
    timeout=5
)

if not file_dialog:
    print("[X] 文件对话框未弹出")
    print("当前窗口:")
    for i in range(1, 10):
        win = auto.WindowControl(searchDepth=1, foundIndex=i)
        if win.Exists(0.2, 0):
            print(f"  [{i}] {win.Name} | {win.ClassName}")
        else:
            break

    # 尝试再次点击
    print("\n尝试再次点击...")
    auto.Click(center_x, center_y)  # 添加图片按钮中心坐标
    time.sleep(2)

    file_dialog = auto.WindowControl(searchDepth=1, ClassName="#32770")
    if not file_dialog.Exists(2, 0):
        print("[X] 仍未找到文件对话框")
        exit(1)

print(f"[OK] 找到文件对话框: {file_dialog.Name}")
file_dialog.SetFocus()
time.sleep(0.5)

# 获取第一张图片
if os.path.exists(image_folder):
    files = [f for f in os.listdir(image_folder) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.bmp'))]
    if files:
        first_image = os.path.join(image_folder, files[0])
        print(f"选择图片: {first_image}")

        # 在文件名输入框输入路径
        edit = file_dialog.EditControl(searchDepth=10)
        if edit.Exists(2, 0):
            edit.Click()
            time.sleep(0.2)

            # 清空并输入
            edit.SendKeys("{Ctrl}a", waitTime=0.1)
            edit.SendKeys(first_image, waitTime=0.02)
            print("[OK] 已输入图片路径")
            time.sleep(0.5)

            # 点击打开
            open_btn = file_dialog.ButtonControl(searchDepth=10, Name="打开(O)")
            if open_btn.Exists(2, 0):
                open_btn.Click()
                print("[OK] 已点击打开按钮")
            else:
                file_dialog.SendKeys("{Enter}")
                print("[OK] 已按 Enter")
            time.sleep(2)
        else:
            print("[X] 未找到文件名输入框")
            exit(1)
    else:
        print(f"[X] 文件夹中没有图片")
        exit(1)
else:
    print(f"[X] 文件夹不存在: {image_folder}")
    exit(1)

# Step 6: 关闭发布页面
print("\n--- Step 6: 关闭发布页面 ---")
time.sleep(1)

sns_window = auto.WindowControl(searchDepth=1, ClassName="mmui::SNSWindow")
if sns_window.Exists(2, 0):
    sns_window.SetFocus()
    time.sleep(0.3)

    # 查找取消按钮
    cancel_btn = sns_window.Control(searchDepth=15, Name="取消")
    if cancel_btn.Exists(2, 0):
        rect = cancel_btn.BoundingRectangle
        auto.Click((rect.left + rect.right) // 2, (rect.top + rect.bottom) // 2)  # 取消按钮中心坐标
        print("[OK] 已点击取消按钮")
        time.sleep(1)

        # 处理确认对话框
        for _ in range(3):
            discard = auto.ButtonControl(searchDepth=5, Name="放弃")
            if discard.Exists(0.5, 0):
                discard.Click()
                print("[OK] 已确认放弃")
                break
            time.sleep(0.3)
    else:
        sns_window.SendKeys("{Escape}")
        print("[OK] 已按 Escape")
        time.sleep(0.5)

# 检查结果
time.sleep(0.5)
sns_window = auto.WindowControl(searchDepth=1, ClassName="mmui::SNSWindow")
if not sns_window.Exists(1, 0):
    print("[OK] 朋友圈窗口已关闭")
else:
    print("[!] 朋友圈窗口仍存在，再次尝试关闭...")
    sns_window.SendKeys("{Escape}")
    time.sleep(0.5)

print("\n" + "=" * 50)
print("流程完成")
print("=" * 50)
